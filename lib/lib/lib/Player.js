...
//操作地圖上的玩家棋子標誌，作出前進動畫。
Player.prototype.moving = function(fromLandMark, toLandMark) {
  if (Meteor.isClient) {
    var player = this;
    //先將玩家棋子的偏移量歸零
    player.setOnMap( fromLandMark.getLatLng() );
    //計算出移動中每一步的位置
    var allLandMark = fromLandMark.getAllLandMarkInPath(toLandMark);
    //再將每一步都拆解為十個座標
    var allLatLng = getAllLatLngBetweenLandMark(allLandMark);
    //紀錄當前步伐
    var currentStep = 0;
    //從一個地標到下一個地標的動畫函數
    var keepGoing = function() {
      var newLatLng = allLatLng[currentStep];
      currentStep += 1;
      if (newLatLng) {
        player.setOnMap( newLatLng );
      }
    };
    //前進第一步
    keepGoing();
    //然後定時執行前進下一步，紀錄定時執行緒的序號
    var intervalId = Meteor.setInterval(keepGoing, CONFIG.waitingMoveTime / 10);
    //將紀錄的定時執行緒序號存入Session中，讓移動結束時，系統可以強制中止動畫
    Session.set('movingInterval', intervalId);
  }
};
if (Meteor.isClient) {
  Session.setDefault('movingInterval', null);
}
//傳入地標陣列構成的路線，將每一步都拆解為十個座標
function getAllLatLngBetweenLandMark(allLandMark) {
  var i;
  var j;
  var fromLandMark;
  var toLandMark;
  ...
  var stepLatLng;
  var result = [];
  for (i = 1; i < allLandMark.length; i += 1) {
    fromLandMark = allLandMark[ i - 1 ];
    toLandMark = allLandMark[ i ];
    fromLatLng = fromLandMark.getLatLng();
    toLatLng = toLandMark.getLatLng();

    latDiff = (toLatLng.lat() - fromLatLng.lat()) / 10;
    lngDiff = (toLatLng.lng() - fromLatLng.lng()) / 10;
    for (j = 1; j <= 10; j += 1) {
      stepLatLng = new google.maps.LatLng(fromLatLng.lat() + latDiff * j, fromLatLng.lng() + lngDiff * j)
      result.push(stepLatLng);
    }
  }
  return result;
}